...
||||||| .r8538
    ...
    LPC_EEPROM->CMD = EEPROM_CMD_ERASE_PRG_PAGE;
=======
    LPC_EEPROM->INT_CLR_STATUS = (0x1 << EEPROM_ENDOF_PROG); // Clear interrupt status
    LPC_EEPROM->ADDR = EEPROM_PAGE_ADDRESS(page_address); // Set address register to page address
    LPC_EEPROM->CMD = EEPROM_CMD_ERASE_PRG_PAGE; // Set command to erase program page
>>>>>>> .r8675
...
void eeprom_erase(uint16_t page_address)
{
    ...
    // Set command to write 32 bit. Note we use 32 bit write to erase the page bec the page is 256 bit and will take less loop iteration
